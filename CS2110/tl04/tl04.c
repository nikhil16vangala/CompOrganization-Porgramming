...
int set_character(struct ascii_image *image, int row, int col, char c) {
    ...
    if (image == NULL || image->data == NULL || row < 0 ||row > image->height-1 || col < 0 || col > image->width-1) {
        return FAILURE;
    }
...
int draw_hollow_box(struct ascii_image *image, int row, int col, int height, int width, char c) {
    ...
    for (int r = row; r < row + height; r++) {
        for (int column = col; column < col + width; column++) {
            if (r == row || r == row + height -1) {
                set_character(image, r, column, c);
            } else {
                if (column == col || column == col + width -1) {
                    set_character(image, r, column, c);
                }
            }
        }
...
struct ascii_image *create_image(int height, int width, char *name) {
    ...
    // These are just to turn off compiler errors initially
    // Please delete once you have implemented the function
    if (name == NULL || height < 1 || width < 1) {
        return NULL;
    }
    struct ascii_image *image1 = malloc(sizeof(struct ascii_image));
    if (image1 == NULL) {
        return NULL;
    }
    image1->name = malloc(strlen(name) + 1);
    if (image1->name == NULL) {
        free(image1);
        return NULL;
    }
    strcpy(image1->name, name);
    image1->data = malloc(height*width);
    if (image1->data == NULL) {
        free(image1->name);
        free(image1);
        return NULL;
    ...
    return image1;    // Don't forget to replace this!
}
...
int add_extension(struct ascii_image *image, char *extension) {
    ...
    char *temp;
    if ((temp = realloc(image->name, strlen(image->name) + strlen(extension) + 1)) == NULL) {
        return FAILURE;
    }
    image->name = temp;
    strcpy(image->name + strlen(image->name), extension);
    return SUCCESS;
}
